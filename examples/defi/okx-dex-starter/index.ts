...
export const OKX_SOLANA_WALLET_ADDRESS =
  process.env.OKX_SOLANA_WALLET_ADDRESS || "";
...
function formatQuoteResult(
  ...
  const fromSymbol =
    quoteData.fromToken?.tokenSymbol ||
    getTokenInfo(fromAddress)?.symbol ||
    fromAddress.slice(0, 8);
  const toSymbol =
    quoteData.toToken?.tokenSymbol ||
    getTokenInfo(toAddress)?.symbol ||
    toAddress.slice(0, 8);

  // Format amounts using the token decimals from the quote
  const toTokenDecimals = parseInt(quoteData.toToken?.decimal || "6");
  const expectedOutput = (
    parseInt(quoteData.toTokenAmount) / Math.pow(10, toTokenDecimals)
  ).toFixed(6);

  // Calculate minimum output (can be adjusted based on slippage)
  const minOutput = (
    (parseInt(quoteData.toTokenAmount) * 0.995) /
  ...
  if (quoteData.quoteCompareList && quoteData.quoteCompareList.length > 0) {
    ...
    quoteData.quoteCompareList.forEach((route: any) => {
      console.log(
        `  ${route.dexName}: ${route.amountOut} ${toSymbol} (Fee: ${route.tradeFee})`,
      );
    });
  }
  ...
  if (
    quoteData.fromToken?.tokenUnitPrice ||
    quoteData.toToken?.tokenUnitPrice
  ) {
    console.log("\nToken Prices:");
    if (quoteData.fromToken?.tokenUnitPrice) {
      console.log(
        `  ${fromSymbol}: $${parseFloat(quoteData.fromToken.tokenUnitPrice).toFixed(4)}`,
      );
    }
    if (quoteData.toToken?.tokenUnitPrice) {
      console.log(
        `  ${toSymbol}: $${parseFloat(quoteData.toToken.tokenUnitPrice).toFixed(4)}`,
      );
    }
  }
}
...
export async function executeSwap(
  ...
  try {
    ...
    if (fromAddress === WRAPPED_SOL_MINT) {
      console.log(
        `Converting from wrapped SOL to native format for API call: ${NATIVE_SOL_MINT}`,
      );
    ...
    if (toAddress === WRAPPED_SOL_MINT) {
      console.log(
        `Converting to wrapped SOL to native format for API call: ${NATIVE_SOL_MINT}`,
      );
    ...
    if (!quote.data || !quote.data[0]) {
      throw new Error(
        `Failed to get valid quote for the swap: ${quote.msg || "No data"}`,
      );
    }
    ...
    const humanFromAmount =
      parseFloat(quoteData.fromTokenAmount) / Math.pow(10, fromDecimal);
    const humanToAmount =
      parseFloat(quoteData.toTokenAmount) / Math.pow(10, toDecimal);
    ...
    try {
      ...
      if (swapError.message?.includes("Non-base58")) {
        ...
        console.log(
          "Agent private key format valid:",
          base58Regex.test(agentPrivateKey.slice(0, 5) + "..."),
        );
      ...
      return {
        ...
        details:
          swapError.response?.data || swapError.responseBody || swapError.stack,
      };
    }
  } catch (error: any) {
...
async function initializeAgent() {
  ...
  try {
    ...
    if (process.env.OKX_SOLANA_WALLET_ADDRESS) {
      const walletAddress = encodeBase58Safe(
        process.env.OKX_SOLANA_WALLET_ADDRESS,
      );
    ...
    console.log(
      ...
      solanaAgent.wallet_address.toString(),
    );
    ...
    const tools = [
      ...
      tool(
        async ({ quote, fromAddress, toAddress, amount }) =>
          executeSwap(solanaAgent, quote, fromAddress, toAddress, amount),
        {
          name: "executeSwap",
...
export async function getQuote(
  ...
  const {
    ...
    humanReadable,
  } = validateAndFormatAmount(amount, fromAddress);
...
async function findToken(
  ...
  for (const [address, info] of Object.entries(tokenInfoCache)) {
    if (
      address.toLowerCase() === query ||
      info.symbol.toLowerCase() === query
    ) {
      return info;
    }
  }
      ...
      if (input.toLowerCase().startsWith("tokens")) {
        ...
        if (searchQuery) {
          ...
          filtered.forEach(([address, info]) => {
            console.log(
              `${info.symbol} (${info.decimals} decimals): ${address}`,
            );
          });
          if (filtered.length === 0) {
            console.log(
              "No matching tokens found. Attempting to discover token...",
            );
            const discovered = await findToken(searchQuery, solanaAgent);
            if (discovered) {
              console.log(
                `Found token: ${discovered.symbol} (${discovered.decimals} decimals): ${discovered.address}`,
              );
            } else {
