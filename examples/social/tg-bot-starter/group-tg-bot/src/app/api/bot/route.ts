...
bot.on("message:text", async (ctx) => {
  ...
  if (!(ctx.chat.type === "private")) {
    if (
      !(ctx.message.text && ctx.message.text.includes(botUsername)) &&
      !(
  ...
  if (!userDocSnap.exists() && ctx.chat.type == "private") {
    ...
    await ctx.reply(
      `Looks like you are using the agent first time. You can fund your agent and start using. Your unique Solana wallet is:`,
    );
    ...
    const keyPair = await getOrCreateUserKeyPair(userId);
    await ctx.reply(
      "Looks like you already have a wallet. Your public key is:",
    );
    await ctx.reply(`${String(keyPair.publicKey)}`);
    await ctx.reply("Your private key is:");
    await ctx.reply(`${String(keyPair.privateKey)}`);
    ...
    await ctx.reply(
      "Looks like you haven't got a wallet yet. Please start a private chat with me to get your wallet.",
