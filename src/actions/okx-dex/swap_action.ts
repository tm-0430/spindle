...
const okxDexSwapAction: Action = {
  ...
  schema: z.object({
    ...
    slippage: z.string().optional().describe("Slippage tolerance (default: 0.001)")
  }),
  handler: async (agent: SolanaAgentKit, input: Record<string, any>) => {
    ...
    // Get quote
    const quote = await getQuote(agent, fromTokenAddress, toTokenAddress, baseUnits.toString(), slippage);

    // Execute swap
    const swapResult = await executeSwap(agent, fromTokenAddress, toTokenAddress, baseUnits.toString(), slippage);
